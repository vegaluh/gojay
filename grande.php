    ...
    if ($verif == false)
        ...
        if ($claim == false)
            ...
            if ($claim == false ) {
                ...
                opsi:
                // echo color("nevy","[?] Sender Rp.1 ? : ");
               
                   
                $token = 'a9f70b42-38a5-4165-b57a-c2a9da31d110';
                $profile = request("/wallet/profile" ,$token);
                $balance = $profile['data']['balance'];
                echo color("green","[+] Sisa Saldo : ".(number_format($balance)))."\n";
            nomor:
            // echo "\n[+] Nomor ? : ";
            $number = $hp;
            $qr = request("/wallet/qr-code?phone_number=%2B$number", "$token");
            if(!isset($qr['data']))
            {
                if($qr['errors'])
                {
            ...
            else
            {
                $qrid = $qr['data']['qr_id'];
                $name = $qr['data']['name'];
                // echo color("nevy","[?] Nominal ? : ");
                $amount = 9000;
                echo color("yellow","[+] Processing Transfer Rp.".number_format($amount)." To ".$number."\n");
                pin:
            ...
            {
                echo color("grey","[+] Transaction ID => ".$tf['data']['transaction_ref']."\n");
